test_that("if no packages, shows nothing", {
  expect_snapshot(cat(tidyomics_attach_message(character())))
})

test_that("message lists all core tidyomics packages", {
  mockr::local_mock(package_version_h = function(x) "1.0.0")
  expect_snapshot(cat(tidyomics_attach_message(core)))
})
